...
# ──────────────────────────────────────────────────────────────
def predict_mood(feature_values: dict, model, scaler, le, feat_cols=None):
    """Always engineers all extra features before passing to scaler."""
    if feat_cols is None:
        feat_cols = ACTIVE_FEATURE_COLS if 'ACTIVE_FEATURE_COLS' in globals() else FEATURE_COLS
    # Always engineer — safe even if already present
